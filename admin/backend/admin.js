...
app.get("/api/mylogs", auth.userAuthenticated, function (req, res) {
  const query =
    "SELECT * FROM products_logs WHERE user_id = ? ORDER BY date DESC";
...
process.on("SIGINT", () => {
  ...
  utils.printMessage(
    ...
    " SERVER ",
    "SHUTDOWN",
    "[0] Received kill signal, shutting down..."
  );
  ...
  sessionStore.close(() => {
    ...
    db.end(() => {
      utils.printMessage(
        ...
        " SERVER ",
        "SHUTDOWN",
        "[2] Connection with database ended"
      );
  ...
  setTimeout(() => {
    utils.printMessage(
      ...
      " SERVER ",
      "SHUTDOWN",
      "Connections took too long to close, forced exit."
    );
